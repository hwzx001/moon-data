...
class multiJson():
    ...
    def getWebPage(self,links):
        try:
            page = requests.get(links,headers=headers,timeout=10,proxies={'http': random.choice(pro),'https':random.choice(pros)}).text
        except Exception as e:
            print(e)
            return ''
        else:
...
# get proxies
def getproxies():
    with open('2020-04-28proxies.txt','r',encoding='utf-8') as f:
        lst=f.read().split('\n')
        for i in range(0,len(lst)-1):
            temp=eval(lst[i])
            if temp['type']=='Http':
                pro.append(str(temp['ip'])+':'+str(temp['port']))
            else:
                pros.append(str(temp['ip'])+':'+str(temp['port']))
...
if __name__ == "__main__":
    getproxies()
    '''
    s=analysisOfProduct('0-11-2.myshopify.com')
    print(s.run())
    '''
    a=getMultiRecord()
    a.run()
